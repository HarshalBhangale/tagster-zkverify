...
const FruitConfirmationScreen = () => {
  ...
  const [isPopupVisible, setIsPopupVisible] = useState(false);
  const [popupTitle, setPopupTitle] = useState("");
  const [verificationStatus, setVerificationStatus] = useState("idle"); // New state for tracking verification status
  const { address } = useAccount();
  const { data: addressData } = useBalance({ address });
  const [bal, setBal] = useState(0);
  const { ready, authenticated } = usePrivy();
  ...
  const account = useAccount();
  const key = "a11aaca3c7025677a643ae70d62cc0e420e6f9a8686986dc6594c4f2f320ad11";
  const sa = privateKeyToAccount(key as `0x${string}`);
  const to = account.address;
  const { data: hash, sendTransaction } = useSendTransaction();
  ...
  const getCategoryData = () => {
    switch (category) {
      case 'banana': return bananaData;
      default: return bananaData;
    }
  };
  ...
  const onTaskComplete = async () => {
    if (currentIndex === 2) {
      ...
      const transactionRequest: SendTransactionVariables<Config, number> = {
        ...
        to: to,
        value: parseEther('0.00001'),
        type: 'eip1559',  // Set a valid type like 'eip1559'
      };
  ...
  // Show the status of proof verification in the popup
  const renderVerificationStatus = () => {
    if (verificationStatus === "verifying") {
      return (
        <div className="flex justify-center items-center">
          <div className="loader border-t-2 rounded-full border-gray-500 bg-gray-300 animate-spin aspect-square w-8 flex justify-center items-center text-yellow-700"></div>
          <p className="mt-2 text-sm text-gray-500">Verifying your proof...</p>
        </div>
      );
    } else if (isConfirmed) {
      return (
